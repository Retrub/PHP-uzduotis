...
if (isset($_POST['submit'])) 
{
			if (!$klaida)
			{
				...
				if (!$Kartojasi)
				{
					$failas = fopen("upload.txt", "a")or die("Unable to open file!");
					fwrite($failas,"\n$_POST[vardas] $_POST[pavarde] $_POST[vidurkis]");
...
{
$failas = fopen("upload.txt", "r")or die("Unable to open file!");
			while(!feof($failas)) {
			$Linija = fgets($failas);
			$Elementas = explode(" ",$Linija);
			$studentas =  new Studentas(
			$Elementas[0],$Elementas[1],$Elementas[2]
		);
		$Kartojasi = false;
				foreach ($_SESSION['Studentai'] as $value) 
				{
